...
export default function AdminDemoPage() {
  ...
  if (!isEnabled) {
    return <main className="card">Admin demo disabled. Set NEXT_PUBLIC_GITHUB_DEMO_ADMIN=true.</main>;
  }

  if (!canOpen) {
    return <main className="card">404</main>;
  }

  return (
    <main className="space-y-4">
      <section className="card">
        <h1 className="mb-2 text-lg font-bold">Admin Demo (GitHub Pages)</h1>
        <p className="text-xs text-zinc-400">Это клиентская заглушка для UI-теста. Здесь нет реального доступа к API/БД.</p>
      </section>

      <section className="card">
        <h2 className="mb-2 font-bold">Users</h2>
        {demoUsers.map((u) => (
          <div key={u.id} className="text-sm">{u.id} — {u.creditsBalance} credits</div>
        ))}
      </section>
      ...
      <section className="card">
        ...
        {demoJobs.map((j) => (
          <div key={j.id} className="mb-2 text-sm">{j.status} — {j.id}</div>
        ))}
      </section>
    </main>
  );
