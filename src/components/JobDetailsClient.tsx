...
import { getJobById, isStaticDemoMode, rerollDemoJob } from '@/lib/static-demo';
...
export function JobDetailsClient({ id }: { id: string }) {
  ...
  useEffect(() => {
    if (!id) return;
    if (isStaticDemoMode()) {
      const sync = () => setJob((getJobById(id) as unknown as Job) ?? null);
      sync();
      const t = setInterval(sync, 1000);
      return () => clearInterval(t);
    }

    fetch(`/api/jobs/${id}`).then((r) => (r.ok ? r.json() : null)).then(setJob).catch(() => setJob(null));
  }, [id]);
